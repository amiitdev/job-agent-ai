...
def is_valid_job(title, description):
    ...
    if not any(k in title_lower or k in desc_lower for k in [
        "react", "node", "express", "mongodb",
        "python", "full stack", "backend"
    ]):
        return False, "Not your stack"

    # ❌ REMOVE IRRELEVANT ROLES (ONLY TITLE)
    ...
    # 🔥 FIX → ONLY CHECK TITLE FOR SENIORITY
    if "senior" in title_lower:
        return False, "Senior rejected"

    if "lead" in title_lower:
        return False, "Lead rejected"

    if any(x in title_lower for x in ["director", "vp", "chief"]):
...
def ai_rank_jobs(jobs):
    prompt = """
You are a strict fresher job recommender.

RULES:
- ONLY select jobs for fresher or <=2 years
- REJECT any Senior, Lead roles
- Prefer Backend, React, Node, Full Stack
- Score strictly between 1–10
- Return ONLY best 5 jobs by ID
...
def print_ui(jobs):
    print("\n" + "="*60)
    print("🔥 FINAL FRESHER JOBS")
    print("="*60)

    if not jobs:
        print("\n😴 No fresher jobs found today")
        return
...
def send_telegram(jobs):
    ...
    if not jobs:
        msg = "😴 No fresher jobs today"
    else:
        msg = "🔥 Fresher Job Alerts 🚀\n\n"
        for j in jobs:
            msg += f"🧠 {j['title']}\n⭐ {j['score']}/10\n🔗 {j['link']}\n\n"
